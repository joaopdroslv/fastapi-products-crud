...
from app.database import dependencies, mongodb, sqlite
...
def test_create_view_log_for_each_product_on_listing(setup_database):
    ...
    for created_product in created_products:
        ...
        assert (
            response_data["number_of_views"] == 10
        )  # Cada produto foi visualizado 10 vezes
...
def test_create_view_log_when_viewing_single_product(setup_database):
    ...
    assert "views" in response_data
    assert (
        response_data["number_of_views"] == 1
    )  # Produto foi pesquisado apenas uma vez
...
def test_update_product_with_invalid_in_stock_status(setup_database):
    ...
    response = client.put(
        f"/products/{created_product_id}", json=invalid_product_to_update
    )
    assert response.status_code == 422
